...
public class RestOAuth2UserServiceTest {
    ...
    public void shouldGetUserInfo() {
        OAuth2AccessToken accessToken = new OAuth2AccessToken(OAuth2AccessToken.TokenType.BEARER, OAuth2TestData.ACCESS_TOKEN, Instant.now(), Instant.now().plusSeconds(10));

        ParameterizedTypeReference<Map<String, Object>> type = new ParameterizedTypeReference<Map<String, Object>>() {};

        when(restOperations.exchange(eq(OAuth2TestData.USER_INFO_URI), eq(GET), requestCaptor.capture(), eq(type)))
                .thenReturn(ResponseEntity.ok(Collections.singletonMap(OAuth2TestData.USERNAME_ATTRIBUTE_NAME, "Scout")));
        ...
        assertThat(oAuth2User.getAttributes()).containsEntry(OAuth2TestData.USERNAME_ATTRIBUTE_NAME, "Scout");
    }
}
